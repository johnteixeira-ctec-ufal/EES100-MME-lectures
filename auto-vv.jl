...
# ╔═╡ c8d553c0-ca46-11ef-028c-31657b27cd3b
begin
	using Plots
...
# ╔═╡ daf64c38-3b18-4374-898c-cadc77eba2b7
begin
	...
	<subtitle>Auto valor e auto vetor</subtitle>
...
O problema de autovalor e autovetor consistem em encontrar um vetor $v\ne 0$ e um escalar $\lambda$, de tal forma que existe uma transformação especial na seguinte forma:

$$Ax = \lambda v$$

onde $A$ é uma matriz quadrada $n\times n$, $v$ é um vetor coluna $n$, e $\lambda$ é um escalar. Qualquer $\lambda$ que satisfaça a equação acima é conhecido como um **autovalor** ou **valor característico** da matriz $A$, enquanto o vetor associado $v$ é chamado de um **autovetor** ou **vetor característico** correspondente a $\lambda$.

A equação de definição do valor e vetor característico pode também ser escrita na forma:

$$(A - \lambda I)v = 0$$

Transformando-se assim em um **sistema linear e homogêneo** de equações que apresenta solução apenas se a matriz for singular, isto é se a matriz $(A - \lambda I)$ é singular, implica em $|(A - \lambda I)| = \mathcal{P}(\lambda) = 0$, onde sendo $\mathcal{P}(\lambda)$ é um polinômio de grau n chamado de **polinômio característico** de A, cujas $n$ raízes são os valores característicos ou autovalores da matriz A.

Expandindo o determinante de $(A - \lambda I)$ observa-se que o único termo de grau $n$ e $n−1$ em relação a $\lambda$ é correspondente ao *produto da diagonal principal* de $(A - \lambda I)$. Portanto igual a 

$$(a_{11} −\lambda)(a_{22}-\lambda)\dots(a_{nn}-\lambda)$$

sendo todos os demais termos de grau inferior a $n-1$. Observa-se também que se $\lambda=0\rightarrow\mathcal{P}(0) = det(A)$, logo o termo independente de $\lambda$ do $\mathcal{P}(\lambda)$ é $det(A)$, portanto:

$$\mathcal{P}(\lambda) = (-\lambda)^n + (a_{11}+a_{22}+\dots+a_{nn})(-\lambda)^{n-1}+\dots+det(A)=0$$

multiplicando por $(-1)^n$, obtemos:

$$\mathcal{P}(\lambda) = \lambda^n + (a_{11}+a_{22}+\dots+a_{nn})\lambda^{n-1}+\dots+(-1)^n det(A)=0$$

Pela expressão notamos que:

(i) o traço da matriz $A$ é: $\text{tr}(A)=a_{11}+a_{22}+\dots+a_{nn} = \lambda_1+\lambda_2+\dots+\lambda_n$

(ii) $\lambda_1\cdot\lambda_2\dots\lambda_n = det(A)$ logo, $\prod_{i=1}^n \lambda_i = det(A)$

Uma importante definição é o conjunto de todos os autovalores de A é chamado de **espectro** de $A$. O espectro consiste em **pelo menos** um autovalor e no máximo $n$ autovalores *numericamente diferentes*. O maior dos valores absolutos dos autovalores de $A$ é chamado de **raio espectral** de $A$.

Portanto o problema de autovalor consistem em:

1. Encontrar os autovalores através das raizes do polinômio $|A-\lambda I|=0$
2. Para cada autovalor $\lambda_i$ encontramos o autovetor correspondente através de $(A−\lambda_i I)v_i = 0$ para $i = 1, 2,\dots, n$
"""

# ╔═╡ 0fb64a8a-9134-4908-b37a-187ece11cf8a
begin
	a = range(-10,10,21);
	md"""
	Podemos ver neste exemplo que o vetor original $v$ é rotacionado e esticado após ser transformado por $A=\begin{pmatrix}2 &0 \\ 0 & 1\end{pmatrix}$. O vetor $v = \begin{pmatrix}1 & 1\end{pmatrix}^T$ é transformado em $\begin{pmatrix}2 & 1\end{pmatrix}^T$. O que acontece quando tentamos fazer o mesmo exercício com um vetor diferente $u=\begin{pmatrix}1 & 0\end{pmatrix}^T$ (autovetor de $A$ referente ao autovalor 2).
	
	v1 : $(@bind v0 Slider(a, show_value=true, default=1))
	v2 : $(@bind v1 Slider(a, show_value=true, default=1))
	"""
end
...
# ╔═╡ 601f834a-bae0-4fcd-9a36-da1d06663307
details(
	md"""**Ex.1.** Determine os autovalores e autovetores de $A=\begin{pmatrix}-5 & 2 \\ 2 & -2\end{pmatrix}$?""",
	md"""Escrevendo em notação matricial $|A - \lambda I| = 0$. Considerando apenas as soluções não triviais, temos:

	$$\mathcal{P}(\lambda) = det\left(\begin{bmatrix}-5-\lambda & 2 \\ 2 & -2-\lambda\end{bmatrix}\right) = (-5-\lambda)(-2-\lambda) - 4 = \lambda^2 +7\lambda + 6 = 0$$

	As soluções desta equação quadrática são $\lambda_1 = -1$ e $\lambda=-6$. Estes são os autovalores de $A$.

	2a. **Autovetor de $A$ correspondente a $\lambda_1$**: Este vetor é obtido partindo de:

	$$(-5-\lambda)x_1 + 2x_2 = 0$$
	$$2x_1 + (-2-\lambda)x_2 = 0$$

	fazendo $\lambda = \lambda_1 = -1$, teremos:

	$$-4x_1 + 2x_2 = 0$$
	$$2x_1 - x_2 = 0$$

	Encontramos que $x_2 = 2x_1$, isso determina um autovetor correspondente a até um múltiplo escalar. Se escolhermos $x_1=1$, obtemos o autovetor $v_1 = (1\quad 2)^T$
	
	2b. **Autovetor de $A$ correspondente a $\lambda_2$**: Para $\lambda = \lambda_2 = -6$, teremos:

	$$x_1 + 2x_2 = 0$$
	$$2x_1 +4 x_2 = 0$$

	Uma solução é $x_1 = -2x_2$ com $x_2$ qualquer(arbitrário). Se escolhermos $x_2=-1$, obtemos assim, um autovetor de A correspondente a $\lambda_2=-6$ é $v = (2\quad -1)^T$.

	
	"""
)

# ╔═╡ f956dd31-3667-4ab7-bbd9-dd3984873da8
md"""
## Métodos numéricos

Em muitos pacotes numéricos existe a função `eig` ou variantes, onde os autovalores e autovetores são obtidos informando a matriz como entrada para função. O método iterativo preferido para encontrar todos os autovalores de uma matriz, **mas não os autovetores ao mesmo tempo**, é o **Método QR**. A ideia é baseada nos dois conceitos a seguir:

1. Matrizes semelhantes terão os mesmos autovalores e autovetores associados. Duas matrizes quadradas $A$ e $B$ são semelhantes se:

$$A = C^{-1}BC$$

sendo $C$ é uma **matriz invertível**.

2. O método QR é uma maneira de decompor uma matriz em duas matrizes $Q$ e $R$, onde $Q$ é uma *matriz ortogonal* e $R$ é uma *matriz triangular superior*.

Juntando estes conceitos podemos encontrar os autovalores, da seguinte forma: Seja uma matriz $A_o$ cujos autovalores devem ser determinados. Na $k$-ésima iteração (começando com $k = 0$), executamos a decomposição QR e obter:

$$A_k = Q_k R_k$$

Desta forma, obtemos:

$$A_{k+1} = R_k Q_k = Q_k^{-1}Q_kR_kQ_k = Q_k^{-1}A_kQ_k$$

Como todos os $A_k$ são semelhantes, como discutimos acima (em 1.), todos eles têm os mesmos autovalores. À medida que a iteração continua, convergimos para uma matriz triangular superior com a forma:

$$A_k = R_kQ_k = \begin{pmatrix}\lambda_1 & \alpha & \dots & \alpha \\ 0 & \lambda_2 & \dots & \alpha\\ \vdots & \vdots & \ddots & \vdots \\0 & 0 & \dots &\lambda_n\end{pmatrix}$$

onde os valores na diagonal principal são os autovalores da matriz. Em cada iteração do método QR, fatorar uma matriz em uma matriz ortogonal e uma triangular superior pode ser feito usando uma matriz especial chamada **matriz Householder**. Não entraremos em detalhes matemáticos sobre como você obtém $Q$ e $R$ da matriz.
"""

# ╔═╡ 298fc57a-9151-4c72-99bb-6416e7b679ac
md"""
Utilizando este algoritmo para determinar os autovalores de $A=\begin{pmatrix}0 & 2\\ 2 & 3\end{pmatrix}$, utilizando o fragmento de código a seguir:
...
Note que após a 4ª iteração, os autovalores são convergidos para os corretos (4 e -1).
"""

# ╔═╡ a9fd1526-7b6f-40bf-9843-aee80046f7dc
md"""
Outros métodos para determinação dos autovalores são o [método de Le Verrier](https://pt.wikipedia.org/wiki/Urbain_Le_Verrier), que determina recursivamente os coeficientes de $\mathcal{P}(\lambda)$ a partir do cálculo dos traços das sucessivas potências de $A$ de $1$ a $n$, seguido de algum método de determinação de raizes de polinômio (para polinômio cúbicos temos a [formula de Cardano](https://pt.wikipedia.org/wiki/Fórmulas_de_Cardano)). Outros métodos são: [método de Leverrier–Faddeev](https://dma.uem.br/kit/topicos-especiais/leverrier-faddeev.pdf), [método de Krylov](https://mat.ufpb.br/~lenimar/textos/krylov.pdf) e outros.
"""

# ╔═╡ f0a51402-2bbd-46d3-b0b4-e534cebd9da9
md"""
## Aplicações do problema de autovalor

O problema de valores característicos têm um número muito grande de aplicações em diversos campos, como engenharia, geometria, física, matemática, biologia, ciência ambiental, economia, psicologia e outras áreas. 

Encontraremos aplicações para Direções principais do tensor de tensões, processos dinâmicos e estocásticos, entre outros.

### Direções principais - Tensor

Em problemas de estabilidade (geo)mecânica de poços, normalmente, estamos interessados em realiza a analise em algum plano (sistema de coordenadas - para poços direcionais) onde o vetor de tensão ($T$ ~ matriz simétrica) se resume à sua componente normal ou em problemas de fraturamento hidráulico/hidro-químico saber a direção preferencial do fraturamento em um ambiente que onde a geotensão é anisotrópica. Nestes exemplos, estamos interessados na direção das tensões principais (autovetores máximo e mínimo) ou determinar os invariantes tensoriais para problemas elasto-plásticos (coeficientes do polínômio característico de $T$).

$$T\cdot v = \lambda v$$

onde as direções principais formam:

$$\sigma = \begin{pmatrix}v_1 & v_2 & v_3\end{pmatrix}$$

sendo $v_1, v_2,\ v_3$ as direções principais ($v_1$ maior e $v_3$ menor), obtidos impondo a restrição de $|v_i| = 1$ para $i=1,2,3$ 
"""

# ╔═╡ 54a7f698-f4ba-45de-9627-befbe20eff4c
begin
	xc = 0. # center x_c or h
	yc = 0. # y_c or k
	ae = 5 # major axis length
	be = 0.5 # minor axis length
	θ = π/4 # angle of major axis with x axis phi or tau

	t = range(0, 2*pi, 50) 
	xe = xc .+ ae.*cos.(t).*cos(θ) .- be.*sin.(t).*sin(θ)
	ye = yc .+ ae.*cos.(t).*cos(θ) .+ be.*sin.(t).*cos(θ)
	plot(xe, ye, c=:black, label=false)
	plot!(xe, xe, c=:red, label="direção principal maior")
	plot!(xe./10, -xe./10, c=:blue, label="direção principal menor", aspect_ratio=:equal)
	xlabel!("\$\\sigma_{xx}\$")
	ylabel!("\$\\sigma_{yy}\$")
end

# ╔═╡ 531571cd-8592-4418-9003-df370e7f9ec9
md"""
### Processos dinâmicos - vibrações

Sistemas massa-mola desempenham um papel importante em problemas dinâmicos e de engenharia. A Figura abaixo mostra tal sistema.

![dyn](https://miro.medium.com/v2/resize:fit:1400/format:webp/1*tkCUQCaE-kG9rTKavRs6bQ.png)

Após as molas serem deformadas, as massas entrarão em movimento oscilatório que a depender da rigidez de cada mola poderão aumentar, diminuir assintoticamente ou entrar em ressonância. Estes comportamentos podem ser caracterizados analisando os autovalores e autovetores resultante do sistema de equações diferencial resultante da modelagem deste sistema, por simplificação ignoramos o amortecimento.

Neste exemplo, ao dividir todo o sistema nos dois blocos e analisando[^1] o diagrama do corpo livre (D.C.L.), através da 2ª Lei de Newton ($\sum F = m a$), chegamos a:

$$m_1\frac{d^2 x_1}{dt^2} = -k_1 x_1 + k_2(x_2 - x_1)$$
$$m_1\frac{d^2 x_1}{dt^2} = -k_2(x_2 - x_1) + k_3 x_2$$

Na forma matricial

$$\underbrace{\begin{bmatrix}m_1 & 0\\ 0 & m_2\end{bmatrix}}_{M}\underbrace{\begin{bmatrix}x_1''\\ x_2'' \end{bmatrix}}_{x''} + \underbrace{\begin{bmatrix}k_1 + k_2 & -k_2\\ -k_2 & k_2+k_3\end{bmatrix}}_{K}\underbrace{\begin{bmatrix}x_1\\ x_2\end{bmatrix}}_{x} = \begin{bmatrix}0\\ 0 \end{bmatrix}$$

Portanto,

$$M x'' + K x = 0$$

As equações acima formam um sistema de equações diferenciais de segunda ordem homogenea acopladas. Para poder resolvê-la precisaremos das condições iniciais apropriadas, para serem usadas para em um solucionador (*solver*) a fim de se obter os deslocamentos das massas como uma função do tempo (isto é, suas oscilações, veremos que precisaremos primeiro converter isso para um sistema de equações diferenciais de primeira ordem fazendo $y_1' =x_1'$ e $y_2' =x_2'$, mas isto fica para mais adiante).

Por hora, para problemas periodicos estacionário $x = a\times\text{sin}(\omega t)$, sendo $a = [a_1\quad a_2]^T$ a amplitude da oscilação das massas, e $\omega$ é a frequência natural não amortecida do sistema. Derivando $x$ e inserindo no sistema de equações diferenciais de segunda ordem homogenea acopladas, obtemos o **problema de autovalor e autovetor generalizado**:

$$\begin{bmatrix}k_1 + k_2 -m_1\omega^2 & -k_2\\ -k_2 & k_2+k_3 -m_2\omega^2 \end{bmatrix}\begin{bmatrix}a_1\\ a_2\end{bmatrix} = \begin{bmatrix}0\\ 0 \end{bmatrix}$$

ou 

$$\left[-\omega^2 M + K\right]a = 0\Rightarrow K a = \omega^2 M a$$

Para uma solução diferente de zero ou não trivial de $a$, isto é $det(K-\omega^2 M) = 0$, que será uma equação quadrática em $\omega^2$. A solução da equação resultante do determinante produz dois valores ($\omega_1^2$ e $\omega_2^2$) para $\omega^2$ sendo $\omega_1$ e $\omega_2$ as frequências naturais. Em outras palavras, há duas frequências naturais para um sistema.

----
[^1] Aqui consideramos o sentido $\rightarrow$ como positivo

"""

# ╔═╡ 04605df9-7d8a-4ee0-b5ce-f0ab78d31485
begin
	k = range(0,2,41);
	m = range(10,400,41);
	Δ = range(-4,4,41);
	md"""
	k1: $(@bind k1 Slider(k, show_value=true, default=2.0))
	k2: $(@bind k2 Slider(k, show_value=true, default=1.25))
	k3: $(@bind k3 Slider(k, show_value=true, default=0.25))
	
	m1: $(@bind m1 Slider(m, show_value=true, default=55))	
	m2: $(@bind m2 Slider(m, show_value=true, default=45))
	
	x1: $(@bind L1 Slider(Δ, show_value=true, default=0.0))	
	x2: $(@bind L2 Slider(Δ, show_value=true, default=-.5))	
	
	"""	
end

# ╔═╡ 27b96c60-4b6f-4625-8a06-df80bc265968
begin
	function twoDOF!(du, u, p, t)
		# Defines the differential equations for the coupled spring-mass system.
		# Arguments:
        #  		u :  vector of the state variables: u = [x1,y1,x2,y2]
        # 		t :  time
        # 		p :  vector of the parameters: p = [m1,m2,k1,k2,k3]
        du[1] = u[1] # x1'
	    du[2] = (- k1 * u[2] + k2 * (u[4] - u[2])) / m1 # x1''
	    du[3] = u[3]
	    du[4] = (- k2 * (u[4] - u[2]) + k3 * u[4]) / m2
	end
	# initial condition
	u0 = [0; L1; 0; L2]
	tspan = (0.0, 700.0)
	prob = ODEProblem(twoDOF!, u0, tspan)
	sol = solve(prob)
	# Plot solution
	plot(sol, idxs = (0, 2), label="\$x_1\$")
	plot!(sol, idxs = (0, 4), yaxis = "displacement (m)", xaxis = "time (sec)", label="\$x_2\$")
	#ylims!(-4,4)
