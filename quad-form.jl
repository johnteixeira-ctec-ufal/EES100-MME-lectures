...
Expressões que geralmente apresentam termos quadráticos, por exemplo para $\mathbb{R}^2$:

$$f(x_1, x_2) = c + b_1x_1 +b_2x_2 + \frac{a_{11}}{2}x_1^2 + a_{12}x_1x_2 + \frac{a_{22}}{2}x_2^2$$

cuja forma matricial é,

$$f(x) = c + b^T x + \frac{1}{2}x^TAx$$

sendo $x = \begin{pmatrix}x_1 & x_2\end{pmatrix}^T$,  $b=\begin{pmatrix}b_1 & b_2\end{pmatrix}^T$, e  $A=\begin{pmatrix}a_{11} & a_{12}\\a_{21} & a_{22}\end{pmatrix}^T$ (simétrica), 

Na forma geral, isto é para $n$ variáveis temos:

$$f(x_1,x_2,\dots,x_n) = c + \sum_{i=1}^nb_ix_i+\frac{1}{2}\sum_{i=1}^n\sum_{j=1}^n a_{ij}x_i x_j$$

Normalmente nos confrontamos com estas formas em problemas de otimização, onde desejamos encontrar extremizantes (máximo ou mínimo). Outras aplicações, residem em problemas elasto-plásticos como a determinação da energia de deformação ($U(d)$), tensão normal (para problemas envolvendo modelos de dano) ($\sigma_n$):

$$U(d) = \frac{1}{2}d^TKd\qquad \sigma_n = n^T\sigma n$$

sendo $d$ vetor deslocamento, $K$ matriz de rigidez, $n$ vetor normal, e $\sigma$ tensor de tensões.

As formas quadráticas são geralmente encontradas em problemas de otimização devido suas propriedades na aplicação da derivada (operador diferencial), pois:

1. Seja o *operador diferencial gradiente* $\nabla$ definido como o operador diferencial cujo componente $i$ é $\nabla_i = \frac{\partial}{\partial x_i}$ que aplicado a $f(x)$ resulta em:

$$\nabla f(x)=\begin{pmatrix}\frac{\partial f(x)}{\partial x_1} \\ \frac{\partial f(x)}{\partial x_1}\\ \vdots \\ \frac{\partial f(x)}{\partial x_n}\end{pmatrix}=b+Ax$$

2. O *operador de Laplace* ou *Laplaciano* (que é o divergente do vetor gradiente) definifo por $\nabla^2 = \nabla^T\nabla = \sum_{i=1}^n\frac{\partial^2}{\partial x_i^2}$, ao aplicarms à função f(x)

$$\nabla^2f(x)=a_{11}+a_{22}+\dots+a_{nn}=\text{tr}(A)$$

3. A *matriz Hessiana* de uma função escalar $f(x)$ como a matriz cujo elemento $ij$ é:

$$\mathcal{H}_{ij}[f(x)]=\frac{\partial^2 f(x)}{\partial x_i\partial x_j}=\frac{\partial^2 f(x)}{\partial x_j\partial x_i}=\mathcal{H}_{ji}[f(x)]$$

O operador $\mathcal{H}$ na realidade é a **matriz Jacobiana do vetor gradiente de uma função escalar** que para a função $f(x)\in\mathbb{R}^2$ temos $\mathcal{H}[f(x)] = A$
"""

# ╔═╡ 9f87d8ea-7270-4e82-9d37-1187f2d0b16e
md"""
É notorio dizer se a matriz $\mathcal{H}[f(x)]=A$, por definição, **é simétrica** a matriz $A$ também deve ser!!. Se uma dada matriz $Q$ não for simétrica, para torná-la simétrica basta fazer a transformação $A\leftarrow\frac{1}{2}\left(Q+Q^T \right)$, pois:

$$x^TAx = x^TQx$$

A forma quadrática geral pode ser simplificada, através de uma translação do eixo, eliminando o termo $b^T x$, considerando $x=z+d$; tem-se

$$b^T x = b^Tz+b^Td$$
$$x^TAx = (z+d)^T(Az+Ad) =z^TAz+d^TAd+z^TAd+d^TAz$$

como a relação $z^TAd=(z^TAd)^T=d^TA^Tz=d^TAz$, pois $A$ é simetríca, portanto:

$$x^TAx=z^TAz+d^TAd+2d^TAz$$

e

$$f(z)=\left(c + b^Td+\frac{1}{2}d^TAd\right) + (b+Ad)^Tz + \frac{1}{2}z^TAz = f(d)+(b+Ad)^Tz+\frac{1}{2}z^TAz$$

Considerando $b+Ad=0\Rightarrow d=-A^{-1}b$, obtemos:

$$f(z)=\underbrace{c}_{f(d)}+\frac{1}{2}z^TAz$$

Finalmente, nesta translação temos $f(0)=c=f(d)$ e o gradiente é:

$$\nabla f(z) = \begin{pmatrix}\frac{\partial f(z)}{\partial z_1}\\ \frac{\partial f(z)}{\partial z_2}\\ \vdots\\ \frac{\partial f(z)}{\partial z_n}\\\end{pmatrix}=Az$$

Deste modo, neste novo sistema de coordenadas, a origem $z = 0$ é um **ponto crítico** que é uma *condição necessária* para o ponto ser um ponto de extremo (máximo ou mínimo) de $f(z)$
"""
...
Se $z=0$ for um *ponto de extremo* de $f(z)$, então para toda a vizinhança de $z=0$ em que $|z|\le\varepsilon$, podemos ter a seguintes situações

1. **Ponto de mínimo** de $f(z)$ para $f(z)>f(0)=c\Rightarrow z^TAz>0$, nesta situação a matriz $A$ caracterizada como **positiva definida**.

2. **Ponto de máximo** de $f(z)$ para $f(z)>f(0)=c\Rightarrow z^TAz<0$, nesta situação a matriz $A$ caracterizada como **negativa definida**.

3. Em qualquer outra situação o ponto *não é nem de máximo ou mínimo*, a matriz é dita ser **não definida** e o ponto crítico um **ponto de sela**.

A forma quadrática pode também ser rescrita na forma análoga à apresentada no processo de transformação de matrizes, assim considerando $z = V w$, em que $V$ é a matriz cujos vetores coluna são os vetores característicos normalizados de $A$ (considerados $n$ vetores característicos linearmente independentes e ortogonais entre si, isto é, os valores característicos são todos reais pois a matriz $A$ é simétrica), a matriz $V$ é ortogonal, isto é, $V^T V = V V^T = I$, portanto:

$$f(w)=c+\frac{1}{2}w^T\left(V^TAV\right)w=c+\frac{1}{2}w^TDw$$

Aqui $c+\frac{1}{2}w^TDw=c+\frac{1}{2}\sum_{i=1}^n\lambda_i w_i^2$, como $z=0$ é um *ponto crítico* $w=0\Rightarrow w = V^{−1} y$, é também um ponto crítico de $f(w)$.

Portanto, podemos caracterizar os pontos críticos segundo:

1. Um *ponto de mínimo* se **todos os valores característicos de $A$ forem positivos**;

2. Um *ponto de máximo* se **todos os valores característicos de $A$ forem negativos**;

3. caso **alguns valores característicos** de $A$ forem positivos e outros negativos o ponto crítico $w = 0 \rightarrow z = 0$, é um *ponto de sela*.
...
Para visualização utilizaremos o $\mathbb{R}^2$ a fim de analisar os pontos críticos. Iniciando com os pontos de extremos (máximo ou mínimo), a forma quadratica mais simples para a análise é

$$f(z) = \alpha_1z_1^2 + \alpha_2z_2^2 + c= \begin{pmatrix}z_1 & z_2\end{pmatrix}\begin{bmatrix}\alpha_1 & 0 \\ 0 & \alpha_2\end{bmatrix}\begin{pmatrix}z_1 \\ z_2\end{pmatrix}$$

Teremos extremos se $\alpha_1$ e $\alpha_2$ apresentarem mesmo sinal (i.e. $det(A)=\alpha_1\alpha_2>0$)

(a) Para um ponto de mínimo se $\alpha_1>0$ e $\alpha_2>0$ 

(b) Para um ponto de máximo se $\alpha_1<0$ e $\alpha_2<0$ 

(c) Para ponto de sela (Nem máximo nem mínimo) se $\alpha_1$ e $\alpha_2$ apresentam sinais distintos (i.e. $det(A)<0$)

(d) Ponto singular (insensível a variações em uma das direções) se $\alpha_1=0$ e $\alpha_2\ne 0$, ou seja, $det(A) = 0$ então a matriz $A$ é **singular**.
"""
...
begin
	a= range(-2,2,21);
	md"""
	α₁: $(@bind α₁ Slider(a, show_value=true, default=-1)) $$\qquad$$
	α₂: $(@bind α₂ Slider(a, show_value=true, default=-1))

	az = $(@bind az Slider(-90:10:90, show_value=true, default=-30)) $$\qquad$$
	dip = $(@bind dip Slider(0:10:90, show_value=true, default=30)) $$\qquad$$
	"""	
end
...
let 
	...
	surface!(plt[1], x,x,z,st=:surface,camera=(az,dip))
	contour!(plt[2], x, x, z, levels=22, color=:turbo)
	scatter!(plt[2], [0], [0], c=:black, ms=4)
	xlabel!("z₀")
	ylabel!("z₁")
	zlabel!(plt[1], "f(z₀,z₁)")
end
